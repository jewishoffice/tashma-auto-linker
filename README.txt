=== Tashma Auto Linker ===
Contributors: jewishoffice
Tags: Tashma, Torah, Jewish
Requires at least: 3.0.1
Tested up to: 6.8
Stable tag: 1.0.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

A Wordpress plugin to easily deploy Tashma's Auto Linker

== Description ==
A Wordpress plugin to easily deploy [Tashma's Auto Linker](https://github.com/Tashma/Tashma-Project/wiki/Tashma-Auto-Linker)

The Tashma auto linker recognizes references on a web page, links them to the corresponding texts on Tashma, and optionally provides a popup containing the text of the references.

It can be run either as a bookmarklet from a browser or embedded into page code. This plugin auto installs it to a Wordpress website.

Options for linker (including its mode, and CSS styles for the pop-up box) can be added via the Settings page for the plugin in the Wordpress admin dashboard.

== Installation ==

1. Download the repository as a ZIP file, or clone it to your desktop using git.
2. **Web Install:** From your site's WordPress dashboard, select "Add New" under the Plugins menu item. Click the "Upload Plugin" button, and upload the plugin's ZIP file.<br/>**FTP Install:** Extract the ZIP file and upload it via FTP to your site's Plugins directory (/wp-content/plugins/).
3. Once installed, click "Activate" to activate the plugin, and set the options on the Settings menu.

